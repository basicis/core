...
class CacheItem implements CacheItemInterface
{
    ...
    public function isHit() : bool
    {
        ...
        if (!is_null($this->value) && ($now->getTimestamp() <= $expiration->add($this->time)->getTimestamp())) {
            return true;
        }
        return false;
    }
    ...
    public function expiresAt($expiration) :  CacheItem
    {
        ...
        $this->expiration = new \DateTime(date('Y-m-d H:i:s', strtotime("30 seconds")));
        return $this;
    }
    ...
    public function expiresAfter($time = null) : CacheItem
    {
        ...
        if ($time instanceof \DateIntervalInterface) {
            $this->time = $time;
            return $this;
        }
        
        if (is_string($time) && preg_match("/^[0-9]{1,} [a-zA-Z]{3,}$/", $time)) {
            $this->time = \DateInterval::createFromDateString($time);
            return $this;
        }
        
        $this->time = \DateInterval::createFromDateString('30 seconds');
        return $this;
    }
}
